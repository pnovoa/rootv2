...
public class PSOHE extends Algorithm {
	...
	private class RBFNetworkEnsemble{
		...
		RBFNetwork<double[]> inverseRBFN;
		RBFNetwork<double[]> multiquadricRBFN;
		
		RBFNetwork.Trainer<double[]> trainer = new RBFNetwork.Trainer<double[]>(new EuclideanDistance());
		
		public RBFNetworkEnsemble(double centroides[][], double datax[][], double datay[]) {
			
			//Gaussian
			GaussianRadialBasis grbf = new GaussianRadialBasis(scalingFactor);
			ThinPlateRadialBasis trbf = new ThinPlateRadialBasis(scalingFactor);
			InverseMultiquadricRadialBasis irbf = new InverseMultiquadricRadialBasis(scalingFactor);
			MultiquadricRadialBasis mrbf = new MultiquadricRadialBasis(scalingFactor);
			
			
			trainer.setRBF(grbf, centroides.length);
			gaussianRBFN = trainer.train(datax, datay, centroides);
			trainer.setRBF(trbf, centroides.length);
			thinplateRBFN = trainer.train(datax, datay, centroides);
			trainer.setRBF(irbf, centroides.length);
			inverseRBFN = trainer.train(datax, datay, centroides);
			trainer.setRBF(mrbf, centroides.length);
			multiquadricRBFN = trainer.train(datax, datay, centroides);
			
			
		}
		
		public double predict(double datax[]) {
			
			double result = gaussianRBFN.predict(datax);
			result += thinplateRBFN.predict(datax);
			result += inverseRBFN.predict(datax);
			result += multiquadricRBFN.predict(datax);
	...
	ArrayList<RBFNetworkEnsemble> approximators;
	double data[][];
	double dataY[];
	int dataIndex=0;
	...
	Particle gBest;
	public String rbfName="ensemble";
	...
	public void init() {
		...
		approximators = new ArrayList<RBFNetworkEnsemble>();
...
//			System.out.println("FUTURE ERROR:\t"+error);
	...
	protected double[] evalInThePast(double x[]) {
		...
		for (int i = 0; i < problem.learningPeriod; i++) {
			...
			result[i] = approximators.get(i).predict(x);
...
//			System.out.println("PAST ERROR:\t"+Math.abs(trueValue - result[i]));
	...
	protected void buildApproximationModel() {
		...
		if(currEnviroment>problem.learningPeriod) {
			...
			approximators.remove(0);
		}
		
		KMeans kmModel = new KMeans(data, K);
		
		RBFNetworkEnsemble ens = new RBFNetworkEnsemble(kmModel.centroids(), data, dataY);
		
		approximators.add(ens);
	}
